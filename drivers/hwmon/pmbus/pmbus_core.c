...
static int pmbus_add_cached_fan_rate(struct pmbus_data *data, int page,
				       int id)
{
...
static int pmbus_find_cached_fan_rate(struct pmbus_data *data, int page,
					int id)
{
	struct pmbus_fan_target_cache *cache;
	             ...
	             u8 config, u8 mask, u16 command)
{
	...
	return pmbus_update_cached_fan_rate(data, page, id, mode, command);
}
			       ...
			       u16 word)
{
	...
	switch (reg) {
		case PMBUS_VIRT_FAN_TARGET_1 ... PMBUS_VIRT_FAN_TARGET_4:
			id = reg - PMBUS_VIRT_FAN_TARGET_1;
			status = pmbus_get_fan_rate(data, page, id, rpm);
			break;
		default:
			status = -ENXIO;
			break;
