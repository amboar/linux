					    ...
					    struct irq_data *data, bool reserve)
{
	...
	if (rc < 0) {
		if (!aspeed_intc1_parent_is_bootmcu(intc1->upstream)) {
			dev_warn(
				intc1->dev,
				"Failed to resolve interrupt route for hwirq %lu in domain %s: %d\n",
				data->hwirq, domain->name, rc);
			return rc;
		}
	...
	for (i = 0; i < intc1->ranges.nranges; i++) {
		...
		for (u32 k = 0; k < r->count; k++) {
			...
			irq_set_chained_handler_and_data(
				irq, aspeed_intc1_ic_irq_handler, intc1);
				 ...
				 struct device_node *parent)
{
	...
	intc1->local = irq_domain_create_linear(
		of_fwnode_handle(node), INTC1_BANK_NUM * INTC1_IRQS_PER_BANK,
