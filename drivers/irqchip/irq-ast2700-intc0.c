	...
	struct aspeed_intc_interrupt_range *resolved)
{
	...
	for (size_t i = 0; i < nc1outs; i++) {
		...
		for (size_t j = 0; j < nc1ranges; j++) {
			 ...
			 */
			if (!(c0domain == irq_find_matching_fwspec(
						  &c1r.upstream,
						  c0domain->bus_token) ||
					 ...
					 unsigned int nr_irqs, void *data)
{
	...
	ret = resolve_parent_route_for_input(intc0, domain->parent->fwnode, (u32)hwirq,
			       &resolved);
	if (ret)
		return ret;
					    ...
					    struct irq_data *data, bool reserve)
{
	...
	if (data->hwirq < INT_NUM) {
		...
		route = resolve_parent_route_for_input(
			intc0, intc0->local->parent->fwnode, data->hwirq, NULL);
		if (route < 0)
			return route;
				 ...
				 struct device_node *parent)
{
	...
	intc0->local = irq_domain_create_hierarchy(
		parent_domain, 0, INT0_NUM, of_fwnode_handle(node),
		&aspeed_intc0_ic_irq_domain_ops, intc0);
	if (!intc0->local)
		return -ENOMEM;
