...
static char* get_defined_signals(const struct aspeed_pin_desc *pdesc)
{
	...
	while ((funcs = *prios)) {
		while ((expr = *funcs)) {
			size_t signal_len = strlen(expr->signal);
			size_t expand = signal_len + 2;

			found = krealloc(found, len + expand + 1, GFP_KERNEL);
			if (!found) {
				return found;
			}
			       ...
			       unsigned int offset)
{
	...
	if (!funcs) {
		...
		WARN(!funcs, "No GPIO signal type found on pin %s (%d). Found: %s\n",
