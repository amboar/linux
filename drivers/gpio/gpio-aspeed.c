...
static inline bool have_bank(struct aspeed_gpio *gpio, unsigned int offset)
{
	return (GPIO_BANK(gpio->config->nr_gpios) < GPIO_BANK(offset));
}
...
{
	const struct aspeed_bank_props *props = find_bank_props(gpio, offset);
	int bit = GPIO_BIT(offset);

	return !(props && props->input & bit);
}
...
static const struct aspeed_bank_props ast2400_bank_props[] = {
	...
	{ 6, 0x0000000f, 0xffffff0f }, /* Y/Z/AA/AB, 4-GPIO hole */
	{ },
};

static const struct aspeed_gpio_config ast2400_config =
	/* 224 for simplicity, actual number is 220 (4-GPIO hole in GPIOY) */
	{ .nr_gpios = 224, .props = ast2400_bank_props, };
